...
SASS는 프로그래밍언어스럽게 작성가능한 preprocrssor
...
class Detail2 extends React.Component {
  //Detail2 컴포넌트가 mount(랜더링) 되었을 때 실행할 코드
  componentDidMount() {}

  //Detail2 컴포넌트가 Unmount(종료) 되기 직전에 실행할 코드
...
x초 후에 코드를 실행하고 싶으면 : setTimeout(function(){실행할코드},ms단위 시간)
...
useEffect(() => {
    let a = setTimeout(() => {
      setalert(false); //setalert를 false
    }, 2000); //2초후 실행
